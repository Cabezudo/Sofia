...
public class ClientManager {
  ...
  private Client create(Connection connection, Person person) throws UserNotExistException, ClusterException {
    String query = "INSERT INTO " + ClientTable.DATABASE_NAME + "." + ClientTable.NAME + " (id) VALUES (?)";
  ...
  public Client get(String address, User owner) throws ClusterException {
    ...
    String query
            ...
            + "LEFT JOIN " + ClientTable.DATABASE_NAME + "." + ClientTable.NAME + " AS a ON p.id = a.id "
  ...
  public ClientList list(Connection connection, Filters filter, Sort sort, Offset offset, Limit limit, User owner) throws UserNotExistException, ClusterException {
    ...
    try {
      ...
      String query = "SELECT SQL_CALC_FOUND_ROWS p.id AS personId, name, lastName, primaryEMailId, address "
              + "FROM " + PeopleTable.DATABASE_NAME + "." + PeopleTable.NAME + " AS p "
              + "RIGHT JOIN " + EMailsTable.DATABASE_NAME + "." + EMailsTable.NAME + " AS e ON primaryEMailId = e.id "
              + "LEFT JOIN " + UsersTable.DATABASE_NAME + "." + UsersTable.NAME + " AS u ON e.id = eMailId "
              + "RIGHT JOIN " + ClientTable.DATABASE_NAME + "." + ClientTable.NAME + " AS a ON p.id = a.id"
